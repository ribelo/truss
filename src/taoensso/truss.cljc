...
(comment
  (let [x 5]      (have    (m/pred integer?) x))
  (let [x 5]      (have    (m/pred string?)  x))
  (let [x 5 y  6] (have odd?  x x x y x))
  (let [x 0 y :a] (have zero? x x x y x))
  (have (m/pred string?)
        (do (println "eval1") "foo")
        (do (println "eval2") "bar"))
  (have (m/pred number?)
        (do (println "eval1") 5)
        (do (println "eval2") "bar")
        (do (println "eval3") 10))
  (have (m/pred pos?) "hello")
  (have (m/pred pos?) (/ 1 0))
  (have (m/pred nil?) false)
  (have nil)
  (have false)
  (have (m/pred string?) :in ["a" "b"])
  (have (m/pred string?) :in (if true  ["a" "b"] [1 2]))
  (have (m/pred string?) :in (if false ["a" "b"] [1 2]))
  (have (m/pred string?) :in (mapv str (range 10)))
  (have (m/pred string?) :in ["a" 1])
  (have (m/pred string?) :in ["a" "b"] ["a" "b"])
  (have (m/pred string?) :in ["a" "b"] ["a" "b" 1])
